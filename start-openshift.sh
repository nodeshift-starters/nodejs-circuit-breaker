...
case "$OSTYPE" in
  darwin*)  open http://`oc get route nodejs-circuit-breaker-greeting | tail -1 | cut -d ' ' -f 4` ;;
  linux*)   xdg-open http://`oc get route nodejs-circuit-breaker-greeting | tail -1 | cut -d ' ' -f 4` ;;
  *)        echo "unknown: $OSTYPE" ;;
esac
